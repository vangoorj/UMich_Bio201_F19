...
knitr::opts_knit$set(root.dir = "~/UMich_Bio201_F19/Lab3")
```
...
scfa_qc <- read_delim(file = "raw_data/SCFA_wkly.txt", 
                        delim = "\t", escape_double = FALSE, trim_ws = TRUE, na=c("NA"),
                        col_types = list())
View(scfa_qc)

# drop median columns
scfa_qc <- select(scfa_qc, -Acetate_median, -Butyrate_median, -Propionate_median)

# keep only individuals who consumed full starch dose
scfa_qc <- filter(scfa_qc, Quantity_compliant == "yes" )

# covert columns to snake case 
scfa_qc <- rename_all(scfa_qc, tolower)
# add units to SCFA measurements 
scfa_qc <- rename(scfa_qc, acetate_mean_mmol_kg = acetate_mean) 
scfa_qc <- rename(scfa_qc, butyrate_mean_mmol_kg = butyrate_mean)
scfa_qc <- rename(scfa_qc, propionate_mean_mmol_kg = propionate_mean)

View(scfa_qc)                   
# calculate total SCFA
scfa_qc <- mutate(scfa_qc, mean_total_scfa_mmol_kg = (acetate_mean_mmol_kg + butyrate_mean_mmol_kg + propionate_mean_mmol_kg))
...
plot_tot <- ggplot(data = scfa_qc, 
       mapping = aes(x = participant_id, 
                     y = mean_total_scfa_mmol_kg)) +
    geom_point(color = "blue") +
    labs(title = "Mean total scfa concentrations of individuals",
         x = "Individuals",
         y = "Total fecal scfa (mmol/kg)") +
    theme(axis.text.x = element_blank())
plot_tot
```
...
> ** The legends are in differnt locations in the facetted figure. There is only one label on the y-axis in the facetted figure rather than it being repeated. The facetted verion can color code weeks without having to specify which color you want each week. ** 
