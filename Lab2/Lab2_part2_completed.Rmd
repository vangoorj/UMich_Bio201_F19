...
```{r}
num_char <- c(1, 2, 3, "a") #class? character 
num_logical <- c(1, 2, 3, TRUE) #class? numeric
char_logical <- c("a", "b", "c", TRUE) #class? character
tricky <- c(1, 2, 3, "4") #class? character
# Why do you think it happens? Because if there is a character present the whole set will be read as a character because you can't perform functions on characters
```

How many values in combined_logical are "TRUE" (as a character) in the following example: 1
```{r}
...
study_sems <- c("winter15", "fall15", "winter16", "fall16", "winter17", "fall17", "winter18", "fall18", "winter19", "fall19")
# Build a vector where the supplements consumed matches the order of the study semesters: 
supplement_sems <- c(
        c("RPS"), # winter15
        c("RMS", "RPS", "Inulin", "Arabinoxylan"), # fall15
        c("RPS", "psyllium_RPS"), # winter16
        c("RPS"), #fall16"
        c("RMS","Inulin", "Accessible"), #winter17
        c("RPS", "HiMaize", "banana"), #fall17
        c("prebiotics", "prebiotics_HiMaize"), #winter18
        c("RPS", "RMS", "RPSx2"), #fall18
        c("RPSx1", "RPSx2"), #winter19
        c("RPS", "oat", "rice", "potato") #fall19
)
# What type are each of these? character

```

```{r}
# Build a vector of the typical age of college students:
college_age <- c(18, 19, 20, 21, 19, 20, 21, 17, 19)
# Confirm this is numeric numeric
class(college_age)
# Convert this vector to character:
college_age_character <- as.character(college_age) 
```
...
mean(pH_1) # mean pH_1 is 7.4
pH1_mean <- mean(pH_1)
round(x = pH1_mean, digits = 2) # rounded pH1_mean is 7.4
pH1_mean <- round(x = pH1_mean, digits = 2)
pH1_mean
pH_2 <- c("8", "7.25", "7.25", "7.75", "7.5", "7.25")
pH_2 <- as.numeric(pH_2)
mean(pH_2) #mean pH_2 is 7.5
pH2_mean <- mean(pH_2)
round(x = pH2_mean, digits = 2) #rounded pH2_mean is 7.5
pH2_mean <- round(x = pH2_mean, digits = 2)
pH1_mean == pH2_mean # FALSE rounded mean pH_1 and rounded mean pH_2 are not equal
```
