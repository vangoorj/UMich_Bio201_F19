...
```{r}
# generate data frame for genus
physq_genus_long <- physq_sub %>%
...
# graph data frame
plot_genus <- ggplot(data = physq_genus_long, 
       aes(x = study_week, #specify the arguments for each axis
           y = abundance, 
...
# save plot
save_plot(filename = "/Users/Owner/Documents/UMich_Bio201_F19/Lab8/figures/plot_genus.pdf",
          plot = plot_genus,
          nrow = 1, ncol = 1, 
          base_aspect_ratio = 5.5)
...
```{r}
# generate data frame for order
physq_order_long <- physq_sub %>%
...
# graph data frame
plot_order <- ggplot(data = physq_order_long, 
       aes(x = study_week, #specify the arguments for each axis
           y = abundance, 
...
# save plot
save_plot(filename = "/Users/Owner/Documents/UMich_Bio201_F19/Lab8/figures/plot_order.pdf",
          plot = plot_order,
          nrow = 1, ncol = 1, 
          base_aspect_ratio = 3.3)
...
```{r}
ordplot_j <- plot_ordination(physeq = physq_sub, 
                     ordination = physq_j, 
                     type = "samples", 
                     color = "semester", 
                     shape = "study_week") +
...
print(ordplot_j)
#save plot
save_plot(filename = "/Users/Owner/Documents/UMich_Bio201_F19/Lab8/figures/plot_ordplot_j.pdf",
          plot = ordplot_j,
          nrow = 1, ncol = 1, 
...
```{r}
#start with same phyloseq object as above 
dat_jaccard <- phyloseq::distance(physq_sub, method = "jaccard") #calculate jaccard index, get distance matrix
...
# run test
adn_res_j <- adonis(formula = dat_jaccard ~ semester * study_week, 
                  data = sampledf)

# view results 
print(adn_res_j)
```
The results show very low p values of 0.001, suggesting that it is statistically significant to group by semester and study week. An R value for grouping by semester is 0.029, which is close to 0 and therefore indicates that grouping of samples by semester is weak. An R value for grouping by study week is very close to 0 (0.008) indicating that grouping samples by study week is also weak. There is likely high variation of samples within each study week and each semester which would explain the low R values despite the low p-values.
