...
f18_but_rps <- scfa_indv %>%
  ...
  filter(notes != "frozen >24hrs") %>%
  filter(sample_wt_g > 0.10) %>% 
  select("participant_id", "sample_id", "butyrate_mM")
...
scfa_indv_qc <- scfa_indv %>%
  rename_all(tolower) %>%
  mutate(ht_m = round((ht_in/35.274), digits = 0), 
         wt_kg = round((wt_lbs*2.205), digits = 0)) %>%
  filter(sample_wt_g > 0.10) %>% 
  filter(notes != "frozen >24hrs") %>%
  mutate(sample_wt_kg = sample_wt_g/1000,
         acetate_mmol_kg = round(((acetate_mm*0.002)/sample_wt_kg), digits = 2),
         butyrate_mmol_kg = round(((butyrate_mm*0.002)/sample_wt_kg), digits = 2),
         propionate_mmol_kg = round(((propionate_mm*0.002)/sample_wt_kg), digits = 2)) %>%
  mutate(total_scfa_mmol_kg = acetate_mmol_kg + butyrate_mmol_kg + propionate_mmol_kg) %>% 
  select(-ht_in, -wt_lbs, -acetate_mm, -butyrate_mm, -propionate_mm, -sample_wt_kg)
...
scfa_wkly_qc <- read_excel("raw_data/SCFA_data.xlsx",
                        ...
                        na = c("", "NA", "-----")) %>% 
  rename_all(tolower) %>% 
  filter(study_week == "week1" | study_week == "week3" | quantity_compliant == "yes") %>% 
  mutate(mean_total_scfa = (acetate_mean + butyrate_mean + propionate_mean)) %>% 
  mutate(median_total_scfa = acetate_median + butyrate_median + propionate_median)
